...
In many problems, using a heap can improve an algorithm's time complexity from *O(n squared)* to *O(n · log n)*, which is a massive improvement (for `n = 1,000,000`, this is `50,000` times faster). A heap is a great option whenever you need to find the maximum or minimum of something repeatedly.
